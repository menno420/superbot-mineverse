# superbot-mineverse — seat digest
...
- `any` · the orchestrator/coordinator seat lacks GitHub MCP tools AND Bash; worker seats have both · coordinator sessions this day could not run git/GitHub operations…
- `any` · direct HTTPS to `api.github.com` is proxy-blocked (sharpens the seed wall above) · verbatim proxy response: 403 "GitHub access is not enabled for this…
