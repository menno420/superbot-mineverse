...
> Live status: `docs/current-state.md`. **Merges are owner-driven on green CI**
> — agents open ready and flag; the owner merges (owner-merges-on-green; see
> `docs/decisions.md` and `CONSTITUTION.md`).
>
...
- The agent auto-merge doctrine + `.github/workflows/auto-merge-enabler.yml`
  — retirement of the workflow file is a kit/adopt-level change (it is
  regenerated by `bootstrap.py` upgrade), so it is left in place here and
  simply not relied on; merges are owner-driven (see `docs/decisions.md`).
